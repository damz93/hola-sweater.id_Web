   ...
   <body>   
     ...
     <?php
		...
		if (isset($_POST['tampil_semua22'])){			
			...
			$data_barang2 = mysqli_query($koneksi,"SELECT * from t_transaksi WHERE ONGKIR>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
		 }
		...
		{			
			...
			if((strcmp($tglnya,$tglakhir)==0) OR empty($tglakhir)){
				...
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi WHERE TGL='".$tglnya."' AND ONGKIR>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
			}
			else{
				...
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi WHERE TGL BETWEEN '".$tgl_awalz."' AND '".$tgl_akhirz."' AND ONGKIR>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");
			}
		}
		...
		{	
			...
			if((strcmp($bulawal,$bulakhir)==0) OR empty($bulakhir)){
				...
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi WHERE MONTH(TGL)='".$bulanaw."' AND ONGKIR>0 AND YEAR(TGL)='".$tahunaw."' GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");	
			}
			else{
				...
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi WHERE TGL between '".$tgl_awal."' AND '".$tgl_akhir."' AND ONGKIR>0 ORDER BY KODE_TRANSAKSI ASC");
			}	
		}
		...
		{	
			...
			if((strcmp($tahunnya,$tahunnya2)==0) OR empty($tahunnya2)){				
				...
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi WHERE YEAR(TGL)='".$tahunnya."' AND ONGKIR>0 GROUP BY KODE_TRANSAKSI ORDER BY KODE_TRANSAKSI ASC");	
			}
			else{
				...
				$data_barang2 = mysqli_query($koneksi, "SELECT * from t_transaksi WHERE TGL between '".$tahunawal_nya."' AND '".$tahunakhir_nya."' AND ONGKIR>0 ORDER BY KODE_TRANSAKSI ASC");	
			}
	...
	?>
      ...
      <table border="1" style="width: 100%" align='left'>
         ...
         <tr>
            ...
            <td align='right'> <?php echo $diskontamp; ?></td>
            <td align='right'> <?php echo $pendp2tamp; ?></td>
         </tr>
		 <tr>
		 <?php
			  }
	   ...
	   ?>
				<tr>
				 ...
				 <tr>
					 ...
					 ?>
					<td colspan="9" align="right"><b>Total Pemasukan Ongkir</td>
				  ...
				  <tr>
					 ...
					 ?>
					<td colspan="9" align="right"><b>Total Keseluruhan</td>
