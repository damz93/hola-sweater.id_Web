...
<html moznomarginboxes mozdisallowselectionprint>
	<head>
		<head>
			...
			<title>Laporan Stok Barang - S W E A T E R I N . M E</title>
	...
	<body>	
      <?php 
		...
		<?php 
			...
			$data_barang = mysqli_query($koneksi,"SELECT * FROM t_stok WHERE QTY>=10 AND NOTES='GUDANG' ORDER BY QTY DESC");
			}
			else if (isset($_POST['barang_juml'])){
			$nama = "QTY Barang - Lebih dari 1";			
			$data_barang = mysqli_query($koneksi,"SELECT * FROM t_stok WHERE QTY>=0 AND NOTES='GUDANG' ORDER BY QTY DESC");
			}
			else if (isset($_POST['barang_tersedikit'])){
			$nama = "QTY Barang - Kurang dari 1";
			$data_barang = mysqli_query($koneksi,"SELECT * FROM t_stok WHERE QTY<=1 AND NOTES='GUDANG' ORDER BY QTY DESC");
			} 
