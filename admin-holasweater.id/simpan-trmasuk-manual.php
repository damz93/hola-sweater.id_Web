<?php
	...
	$olehyy        = $_SESSION['username'];	
			...
			$data_tr = mysqli_query($koneksi,"SELECT ID,KODE_TRANSAKSI FROM t_stok_masuk ORDER BY ID DESC LIMIT 1");
			 ...
			 while($d = mysqli_fetch_array($data_tr)){
				//$jumtranskX        = $d['ID'];				
				$jumtranskX        = substr($d['KODE_TRANSAKSI'],6);		
			 }
	...
	$sql = mysqli_query($koneksi, "select * from t_stok_masuk_temp WHERE OLEH='" . $olehyy . "'");	
	while ($data = mysqli_fetch_array($sql)) {
			...
			$kuantitas    = $data['QTY'];
				...
				$query = "INSERT INTO t_stok_masuk(KATEGORI,TERIMA_DARI,KODE_TRANSAKSI,KODE_BARANG,JENIS_BARANG,SIZE_,WARNA,QTY,TGL,WAKTU,OLEH,KETERANGAN)VALUES('$kategori','$olehh','$kode_masuk','$kode_barang','$jenis_barang','$sizee','$warna','$kuantitas','$tgl','$waktu','$oleh','$keterangan')";
	...
	$sql2 = "DELETE FROM t_stok_masuk_temp where OLEH='" . $olehyy . "'";
	if (mysqli_query($koneksi, $sql2)) {
		echo "<script>alert('data tersimpan');window.location.href='barang-masuk-new';</script>";
	} else {
