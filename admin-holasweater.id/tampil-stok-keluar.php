...
<html>
   <head>
      ...
      <script>
         function sebelum() {         	
           var noref = document.getElementById("noreff").value; 
           if(noref==""){
         	  alert('Masukkan No.Reff');	
         	  document.getElementById("noreff").focus();
         	  return false;
           }
         	else{
         return confirm('Yakin ingin simpan?');         	 
   ...
   <?php
		include 'koneksi.php';
			error_reporting(0);
		//$data_tr = mysqli_query($koneksi,"select max(ID) as ID from t_stok_keluar");
		$data_tr = mysqli_query($koneksi,"SELECT ID,KODE_TRANSAKSI_AUTO FROM t_stok_keluar ORDER BY ID DESC LIMIT 1");
				 while($d = mysqli_fetch_array($data_tr)){
			      ...
			      if ($jumtranskX == 0) {
			      	$kode_penjualan = "OUTS-0000000001";
			      }
			      else{
			      	$jumtranskX++;
					if (strlen($jumtranskX)== 1){
			      		$kode_penjualan = "OUTS-000000000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 2){
			      		$kode_penjualan = "OUTS-00000000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 3){
			      		$kode_penjualan = "OUTS-0000000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 4){
			      		$kode_penjualan = "OUTS-000000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 5){
			      		$kode_penjualan = "OUTS-00000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 6){
			      		$kode_penjualan = "OUTS-0000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 7){
			      		$kode_penjualan = "OUTS-000".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 8){
			      		$kode_penjualan = "OUTS-00".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 9){
			      		$kode_penjualan = "OUTS-0".$jumtranskX;
			      	}
			      	else if (strlen($jumtranskX)== 10){
			      		$kode_penjualan = "OUTS-".$jumtranskX;
			      	}
			      }
	?>
      ...
      <form method="post" onsubmit="return sebelum()" id="form_tampil" name="myff2" action="simpan-trkeluar.php">
         ...
         <div class="form-group">
					...
					<table border="0" class="table" cellpadding="2" cellspacing="2" align=center>
						<tr>
							<th colspan="4">
							<p align='left' style="color:#31859b;font-size:20pt;"><b>Input Barang Keluar</b></p>
							</td>
						</tr>						
						<tr>
							<td align="left">
							...
							<td>
								<select  class="form-control form-control-sm" name="costumerx" id="costumerx"  onchange="autofocus2()" autofocus>
						...
						<tr>	
							...
							<td>								
								<input type="text" maxlength="25" value="" id="noreff" name="noreff" class="form-control form-control-sm ">
							</td>							
							<td align="right">
								Kode Pengeluaran Barang
							</td>
							<td>								
								<input type="text" readonly="readonly"value="<?php echo $kode_penjualan; ?>" id="kode_transaksii" name="kode_transaksii" class="form-control form-control-sm ">
							</td>
						</tr>
               ...
               <table border="0" class="table" cellpadding="2" cellspacing="2" align=center>
                  <div class="form-group">
                     ...
                     <tr>
                        ...
                        <td colspan="4">
                          <button width="100%" type="reset" value="reset" class="btn btn-danger btn-lg btn-block">Cancel</button>
						</td>
						<td colspan="2">
