...
<html>
	<head>
		...
		<title>Data Order Costum - S W E A T E R I N . M E</title>
		...
		<div class="bg">
			...
			<table id="tabel1" class="table table-striped" border="1" cellpadding="0" cellspacing="1">
				...
				<?php 
					...
					if($_SESSION['level']=="DESIGNER") {
						$data = mysqli_query($koneksi, "select COUNT('QTY') AS JUMLAHH,KODE_ORDER,TGL,STATUS from t_order WHERE STATUS='DESIGNER' GROUP BY KODE_ORDER ORDER BY ID DESC");
					}
					else if($_SESSION['level']=="OPERATOR") {
						$data = mysqli_query($koneksi, "select COUNT('QTY') AS JUMLAHH,KODE_ORDER,TGL,STATUS from t_order WHERE STATUS='OPERATOR' GROUP BY KODE_ORDER ORDER BY ID DESC");
					}
					else{
						$data = mysqli_query($koneksi, "select COUNT('QTY') AS JUMLAHH,KODE_ORDER,TGL,STATUS from t_order GROUP BY KODE_ORDER ORDER BY ID DESC");
					}
