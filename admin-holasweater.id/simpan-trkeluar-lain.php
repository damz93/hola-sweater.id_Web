<?php
	...
	$olehh = $_POST['oleh_lain'];	
	$detail = $_POST['detail_lain'];	
	$olehyy        = $_SESSION['username'];	
		...
		if ($detail == "GIVEAWAY"){
			...
			$data_tr = mysqli_query($koneksi,"SELECT ID,KODE_TRANSAKSI_AUTO FROM t_stok_keluar_retur ORDER BY ID DESC LIMIT 1");
		}
	...
	while ($data = mysqli_fetch_array($sql)) {
			...
			//$kode_trx             = $data['KODE_TRANSAKSI'];
			
			
			if ($detail != "GIVEAWAY"){
				$query = "INSERT INTO t_stok_keluar_retur(OLEH2,PELANGGAN,KODE_TRANSAKSI_AUTO,KODE_TRANSAKSI,KODE_BARANG,JENIS_BARANG,SIZE_,WARNA,QTY,TGL,WAKTU,OLEH,KETERANGAN)VALUES('$olehh','$pelanggan','$kode_pengeluaran','$detail','$kode_barang','$jenis_barang','$sizee','$warna','$kuantitas','$tgl','$waktu','$oleh','$keterangan')";	
			}
			else{
